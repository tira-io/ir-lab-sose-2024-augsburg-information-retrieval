...
def generate_expansion(query):
    inputs = tokenizer(f"Expand the following query: {query}", return_tensors="pt")
...
for index, row in topics_df.iterrows():
    ...
    expanded_queries.append({"query_id": row['qid'], "llm_expansion": expanded_query})

# Save to JSONL file
with open('query-expansion/queries_expanded.jsonl', 'w') as f:
